  ...
  double average2 = sum / NUM_VALUES;
    ...
    SmartDashboard.putNumber("Sharp Sensor Average2", average2);
      ...
      if (camera.read(frame)) {
        ...
        ByteBuffer buffer = ByteBuffer.allocateDirect(gray.rows() * gray.cols());
        gray.get(0, 0, buffer.array());

        // Use detect ou método alternativo disponível no 2025
        AprilTagDetection[] detections = detector.detect(frame);

        for (AprilTagDetection detection : detections) {
          SmartDashboard.putNumber("AplilTag", detection.getId());
          } 
          
          }
  }
  private void executarComando(int id) {
    switch (id) {
        case 0 -> andarFrente();
        case 1 -> andarTras();
        case 2 -> virarEsquerda();
        case 3 -> virarDireita();
        default -> parar();
    }
}
...
private void andarFrente() {
    motorEsquerdaMestre.set(0.5); motorDireitaMestre.set(0.5);
}
...
private void parar() {
    motorEsquerdaMestre.set(0.0); motorDireitaMestre.set(0.0);
}
...
// Esse é o autonomus que será utilizado caso o robô comece no meio
  ...
  double average2 = sum / NUM_VALUES;
    ...
    public void teleopPeriodic() {
      ...
      boolean y = controle.getRawButton(6);
      boolean a = controle.getRawButton(4);
      boolean b = controle.getRawButton(9);
      boolean lb = controle.getRawButton(5);
      boolean rt = controle.getRawButton(3);
      boolean x = controle.getRawButton(8);
      boolean lt = controle.getRawButton(1);
      boolean c = controle.getRawButton(2);
      boolean rb = controle.getRawButton (7);
      boolean quickTurn = controle.getRawButton(11);
      int pov = controle.getPOV();
...
/* 
    ...
    double speed = deadband(-controle.getRawAxis(1), 0.055); /* Define funcionalidade do botão analógico esquerdo */
    double turn = deadband(-controle.getRawAxis(0), 0.055); /* Define funcionalidade do botão analógico direito */
